...
from utils import normalize_point_coords, find_grid_indices, get_gaussian_blob_new
...
class ComputeUncertainty:
    ...
    def get_unc_nerfacto(self, ray_bundle, model):
        ...
        semantics = model.renderer_semantics( field_outputs[FieldHeadNames.SEMANTICS], weights=semantic_weights)
        #semantics = torch.sigmoid(semantics)
    ...
    def main(self) -> None:
        ...
        for step in range(len_train):
            ...
            if not isinstance(pipeline.model, MipNerfModel):
                ...
                hessian = self.find_uncertainty(points, offsets, outputs['semantics'].repeat(1,3), pipeline.model.field.spatial_distortion)
