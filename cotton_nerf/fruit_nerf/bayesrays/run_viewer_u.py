...
from utils import find_grid_indices
import torch.nn.functional as F
from output_uncertainty import get_uncertainty
from fruit_nerf.fruit_nerf import FruitModel
...
def get_output_nerfacto_new(self, ray_bundle):
    ...
    masked_density = semantic_projection(field_outputs, field_outputs[FieldHeadNames.DENSITY])
    semantic_weights = ray_samples.get_weights(masked_density)
    semantics = self.renderer_semantics(field_outputs[FieldHeadNames.SEMANTICS], weights=semantic_weights)
    depth = self.renderer_depth(weights=weights, ray_samples=ray_samples)
    accumulation = self.renderer_accumulation(weights=weights)

    # semantics colormaps
    semantic_labels = torch.sigmoid(semantics)
    threshold = 0.9
    semantic_labels = torch.heaviside(semantic_labels - threshold, torch.tensor(0.)).to(torch.long)
    semantics_colormap = self.colormap.to(self.device)[semantic_labels].repeat(1, 3)
...
def get_output_fn(model):
    ...
    elif isinstance(model, FruitModel):
        return get_output_nerfacto_new
...
class RunViewerU:
    ...
    def _start_viewer(self, config: TrainerConfig, pipeline: Pipeline, step: int):
        ...
        if nerfstudio_version >= "0.3.1":
            ...
            viewer_state.train_btn_state = "completed"
